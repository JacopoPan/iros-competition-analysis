...
import sys
# sys.path.append('h2/sim2real_evaluation')
sys.path.append('arg/sim2real_evaluation')
# sys.path.append('eku/sim2real_evaluation')
...
def run(test=False):
    ...
    # Load in static vicon objects 
    path = '/home/dsl25/Desktop/cf_simreal_rosbag/vicon_obs.pkl'
    import pickle
    overwrite = pickle.load(open(path, 'rb'))

    config.quadrotor_config.gates = overwrite['gates']
    config.quadrotor_config.obstacles = overwrite['obstacles']

    log_output_path = 'results/arg'
    ...
    logger = Logger(logging_freq_hz=CTRL_FREQ, output_folder=log_output_path)
    episodes_count = 1
    cumulative_reward = 0
    collisions_count = 0
